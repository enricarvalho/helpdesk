...
export default function ListaChamados({ 
  ...
  ...props 
}) {  const [filtros, setFiltros] = React.useState({ busca: '', status: '', prioridade: '', responsavel: '', dataInicio: '', dataFim: '' });
  ...
  const chamadosFiltrados = filtrarPorAba(chamados.filter(c => {
    ...
    if (match && filtros.responsavel) {
      match = (c.atribuido === filtros.responsavel || c.responsavel === filtros.responsavel);
    }
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems="center" mb={2}>
            ...
            >
              ...
              {usuarios && usuarios.map(u => (
                <MenuItem key={u.email} value={u.email}>{u.nome}</MenuItem>
              ))}
            </TextField>
            <TextField 
              type="date" 
            ...
            <Button 
              variant="outlined" 
              color="warning" 
              onClick={() => setFiltros({ busca: '', status: '', prioridade: '', responsavel: '', dataInicio: '', dataFim: '' })}
            >
              Limpar filtros
            </Button>
          </Stack>
