...
export default function ListaChamados({ 
  ...
  const [modalResolucao, setModalResolucao] = React.useState(false);
  const [chamadoParaResolver, setChamadoParaResolver] = React.useState(null);
  const [comentarioResolucao, setComentarioResolucao] = React.useState('');
  ...
  // Estados para abas do modal de detalhes
  const [abaModalDetalhes, setAbaModalDetalhes] = React.useState(0);
  ...
  // Novo useEffect para atualizar o modal de detalhes quando um chamado é modificado globalmente
  React.useEffect(() => {
    const handleChamadoAtualizado = (event) => {
      if (event.detail && detalhe && event.detail._id === detalhe._id) {
        // Atualiza o estado 'detalhe' com os dados mais recentes do evento
        // Isso garante que o modal, se estiver aberto para este chamado, reflita as mudanças.
        setDetalhe(prevDetalhe => ({ ...prevDetalhe, ...event.detail }));
      }
    };

    window.addEventListener('chamadoAtualizado', handleChamadoAtualizado);

    return () => {
      window.removeEventListener('chamadoAtualizado', handleChamadoAtualizado);
    };
  }, [detalhe]); // Adiciona 'detalhe' como dependência
  ...
  // Funções para modal de resolução
  const abrirModalResolucao = (chamado) => {
    setChamadoParaResolver(chamado);
    setComentarioResolucao(''); // Limpa o campo ao abrir
    setModalResolucao(true);
  };

  const fecharModalResolucao = () => {
  ...
  const confirmarResolucao = () => {
    if (!comentarioResolucao.trim()) {
      setToast({ open: true, msg: 'Por favor, descreva a solução aplicada.', type: 'warning' });
      return;
    }
    if (onStatus && chamadoParaResolver) {
      // A função onStatus no componente pai (Dashboard.js ou similar)
      // precisará ser ajustada para lidar com 'comentarioResolucao'.
      onStatus(chamadoParaResolver, 'Finalizado', comentarioResolucao);
    }
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', mt: 2 }}>
        ...
        <Paper sx={{ p: 3, mb: 3, borderRadius: 3, boxShadow: 3 }}>
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems="center" mb={2}>
            ...
            >
              ...
              <MenuItem value="Alta">Alta</MenuItem>              <MenuItem value="Urgente">Urgente</MenuItem>
            </TextField>
            <TextField 
              type="date" 
              label="Data inicial" 
                        ...
                        {(c.atribuido || c.responsavel) && c.status !== 'Finalizado' && (
                          <Tooltip title="Finalizar">
                            <IconButton 
                              ...
                              onClick={e => { 
                                e.stopPropagation(); 
                                // Em vez de chamar onStatus diretamente, abre a modal de resolução
                                abrirModalResolucao(c);
                              }}
                            >
      ...
      <Dialog open={!!detalhe} onClose={() => { setDetalhe(null); setAbaModalDetalhes(0); }} maxWidth="lg" fullWidth>
        ...
        <DialogContent dividers sx={{ background: '#f8fafc', p: 0 }}>
          {detalhe && (
            <Box sx={{ width: '100%' }}>
              <Tabs value={abaModalDetalhes} onChange={(e, newValue) => setAbaModalDetalhes(newValue)} sx={{ borderBottom: 1, borderColor: 'divider', background: '#fff' }}>
                <Tab label="Detalhes" />
              ...
              {abaModalDetalhes === 0 && (
                <Box sx={{ p: 2 }}>
                  <Stack spacing={2}>
                    ...
                    {/* Exibir comentário de resolução SE o chamado estiver Finalizado e o comentário existir */}
                    {detalhe.status === 'Finalizado' && detalhe.comentarioResolucao && (
                      <>
                        <Divider sx={{ my: 2 }} />
                        <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: '#2e7d32' }}>
                          Resolução do Chamado:
                        </Typography>
                        <Paper elevation={0} sx={{ p: 1.5, background: '#e8f5e9', borderRadius: 1, borderLeft: '4px solid #4caf50' }}>
                          <Typography variant="body2" sx={{ whiteSpace: 'pre-line' }}>
                            {detalhe.comentarioResolucao}
                          </Typography>
                        </Paper>
                      </>
                    )}
                    
                    <Divider sx={{ my: detalhe.status === 'Finalizado' && detalhe.comentarioResolucao ? 1 : 2 }} />
                    <Typography variant="subtitle2">Histórico de Comentários</Typography>
              ...
              {abaModalDetalhes === 1 && isAdmin && (
                <Box sx={{ minHeight: 400 }}>
                  <AIAssistant 
                    chamadoId={detalhe._id || detalhe.id}
                    isOpen={abaModalDetalhes === 1}
                    onClose={() => setAbaModalDetalhes(0)}
                  />
                </Box>
              )}
            </Box>
        ...
        <DialogActions>
          <Button onClick={() => { setDetalhe(null); setAbaModalDetalhes(0); }}>Fechar</Button>
        </DialogActions>
      </Dialog>
      
      {/* Modal de Resolução */}
      <Dialog open={modalResolucao} onClose={fecharModalResolucao} maxWidth="sm" fullWidth>
        <DialogTitle>Finalizar Chamado #{chamadoParaResolver?.numeroChamado}</DialogTitle>
        <DialogContent>
          <Typography variant="body1" gutterBottom>
            Por favor, descreva a solução aplicada para o chamado "{chamadoParaResolver?.titulo}".
          </Typography>
          <TextField
            autoFocus
            margin="dense"
            id="comentarioResolucao"
        ...
        <DialogActions>
          <Button onClick={fecharModalResolucao}>Cancelar</Button>
          <Button onClick={confirmarResolucao} variant="contained" color="primary">
            Confirmar Resolução
          </Button>
        </DialogActions>
      </Dialog>
  ...
  async function handleEnviarComentario() {
    ...
    try {
      ...
      // Não adiciona o comentário localmente para evitar duplicidade
      setComentario('');
      setAnexos([]);
      setToast({ open: true, msg: 'Comentário enviado!', type: 'success' });
      // O histórico será atualizado pelo evento socket chamadoAtualizado
    } catch (err) {
...
export function HistoricoChamados({ 
  ...
  const [filtros, setFiltros] = React.useState({ busca: '', prioridade: '', dataInicio: '', dataFim: '' }); // Removido status do estado inicial de filtros
  const [detalhe, setDetalhe] = React.useState(null);
  ...
  const chamadosFiltrados = chamados.filter(c => {
    // Força o status para Finalizado para esta visualização
    ...
    if (!dataOriginalChamado) return true; // Se não houver data, não filtra por data

    const dataChamado = new Date(dataOriginalChamado);
    if (isNaN(dataChamado.getTime())) return true; // Data inválida, não filtra por data

    // Normaliza a data do chamado para o início do dia
    dataChamado.setHours(0, 0, 0, 0);

    let dataInicioFiltro = null;
    if (filtros.dataInicio) {
      // A data do filtro já vem como 'YYYY-MM-DD'
      dataInicioFiltro = new Date(filtros.dataInicio + 'T00:00:00'); 
      if (isNaN(dataInicioFiltro.getTime())) dataInicioFiltro = null;
      else dataInicioFiltro.setHours(0,0,0,0); // Redundante, mas garante
    }

    let dataFimFiltro = null;
    if (filtros.dataFim) {
      // A data do filtro já vem como 'YYYY-MM-DD'
      dataFimFiltro = new Date(filtros.dataFim + 'T00:00:00');
      if (isNaN(dataFimFiltro.getTime())) dataFimFiltro = null;
      else dataFimFiltro.setHours(23, 59, 59, 999); // Ajusta para o final do dia
    }
    
    let match = true;
    if (busca) match = (c.titulo?.toLowerCase().includes(busca) || c.descricao?.toLowerCase().includes(busca) || (c.numeroChamado && c.numeroChamado.toString().toLowerCase().includes(busca)));
    
    if (match && filtros.prioridade) match = c.prioridade === filtros.prioridade;
    
    // Lógica de filtro de data
    if (match && dataInicioFiltro && dataFimFiltro) {
      match = dataChamado >= dataInicioFiltro && dataChamado <= dataFimFiltro;
    } else if (match && dataInicioFiltro) {
      match = dataChamado >= dataInicioFiltro;
    } else if (match && dataFimFiltro) {
      match = dataChamado <= dataFimFiltro;
    }
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', mt: 4 }}>
      <Paper sx={{ p: 3, mb: 3, borderRadius: 3, boxShadow: 3 }}>
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems="center" mb={2}>
          <TextField label="Busca (Título, Descrição, Nº)" name="busca" value={filtros.busca} onChange={e => setFiltros(f => ({ ...f, busca: e.target.value }))} size="small" sx={{ minWidth: 180 }} />
          <TextField 
            select 
            label="Prioridade" 
            name="prioridade" 
          ...
          <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={ptBR}>
            <DatePicker
              label="Data inicial"
              value={filtros.dataInicio ? new Date(filtros.dataInicio + 'T00:00:00') : null} 
              onChange={date => setFiltros(f => ({ ...f, dataInicio: date ? date.toISOString().split('T')[0] : '' }))}
              renderInput={(params) => <TextField {...params} size="small" sx={{ minWidth: 140 }} InputLabelProps={{ shrink: true }} />}
            />
            <DatePicker
              label="Data final"
              value={filtros.dataFim ? new Date(filtros.dataFim + 'T00:00:00') : null} 
              onChange={date => setFiltros(f => ({ ...f, dataFim: date ? date.toISOString().split('T')[0] : '' }))}
              renderInput={(params) => <TextField {...params} size="small" sx={{ minWidth: 140 }} InputLabelProps={{ shrink: true }} />}
            />
          ...
          <Button 
            variant="outlined" 
            color="warning" 
            onClick={() => setFiltros({ busca: '', prioridade: '', dataInicio: '', dataFim: '' })}
          >
            Limpar filtros
          </Button>
        </Stack>
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell>Data</TableCell>
                <TableCell>Título</TableCell>
                <TableCell>Status</TableCell> {/* Nova coluna Status */}
                <TableCell>Prioridade</TableCell>
                <TableCell>Responsável</TableCell>
                <TableCell>Ações</TableCell>
              </TableRow>
            ...
            <TableBody>
              {chamadosFiltrados.length === 0 ? (
                <TableRow><TableCell colSpan={7}><Typography align="center" color="text.secondary">Nenhum chamado finalizado encontrado.</Typography></TableCell></TableRow> /* Ajustado colSpan */
              ) : chamadosFiltrados.map((c, idx) => (
                <TableRow key={c._id || c.id || idx} hover onClick={() => setDetalhe(c)} style={{ cursor: 'pointer' }}>
                  <TableCell>{(c.numeroChamado || '-').replace(/^CH-/, '')}</TableCell>
                  <TableCell>{(c.data || c.criadoEm || c.updatedAt) ? new Date(c.data || c.criadoEm || c.updatedAt).toLocaleDateString('pt-BR') : '-'}</TableCell>
                  <TableCell>{c.titulo}</TableCell>
                  <TableCell><Chip label={c.status} color="success" size="small" /></TableCell> {/* Nova célula para Status */}
                  <TableCell><Chip label={c.prioridade} color={prioridadeCor[c.prioridade] || 'default'} size="small" /></TableCell>
                  <TableCell>{usuarios?.find(u => u.email === (c.atribuido || c.responsavel))?.nome || c.atribuido || c.responsavel || '-'}</TableCell>
                  <TableCell>
                    <Button size="small" variant="outlined" onClick={e => { e.stopPropagation(); setDetalhe(c); }}>Detalhes</Button>
                  </TableCell>
