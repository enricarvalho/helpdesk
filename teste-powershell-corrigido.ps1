# Teste PowerShell para API de email - CORRIGIDO
...
try {
    ...
    Write-Host ""
    Write-Host "📧 Verificação de templates:" -ForegroundColor Cyan
...
$saveBody = @{
    ...
    templates = @{
        novoChamado = @{
            assunto = "TESTE PowerShell - Novo Chamado #{numeroChamado}"
            corpo = "TESTE PowerShell - Este é um teste de template de chamado."
        }
    }
...
try {
    ...
    if ($configVerifica.templates.novoChamado.assunto -like "*TESTE PowerShell*") {
        Write-Host "🎉 SUCESSO! Template foi salvo corretamente!" -ForegroundColor Green
    } else {
        Write-Host "❌ FALHA! Template não foi salvo." -ForegroundColor Red
    }
